...
- hardcoded "view" variable name (not configurable anymore)
- added $datetime ProcessWire api var
- switched to string module version numbering
- merged additional macros and filters checkboxes in module settings
- filters rewrite: use static methods of a new Helpers class (which can be used separately from the module)
- new filter "mergeWithArray"

- splitted filter "append" to "append", "appendToArray"
- renamed filter "contains" to "stringContains"
- renamed filter "bodyclass" to "pageClasses"
- renamed filter "active" to "isActive", returns boolean instead string
- renamed filter "default" to "emptyValue"
- renamed filter "niceurl" to "formatUrl"
- renamed filter "list" to "makeList"
- renamed filter "children" to "getChildren"
- renamed filter "imageattrs" to "imageAttributes"
- renamed filter "first" and "last" to "getArrayValue" (with first, last and numeric parameters)
- renamed filter "firstkey" and "lastkey" to "getArrayKey" (with first, last and numeric parameters)

- getlines/getline filter: switched to ini format (allows multiline values)
- getlines/getline filter: fixed fallback to default language
- getlines/getline filter: added $useFallback parameter
- srcset filter: disable automatic "x0" suffix
- srcset filter: "original" can be added anywhere in a set (not only as first)
- removed filter "truncatehtml" in favor of the built-in sanitizer
- removed filter "localname" (recent ProcessWire versions has the same feature)
- removed filter aliases: children, pager, sanitizer
- added Nette Tester tests for filters
- required PHP version is 7
